...
mod tests {
    ...
    async fn spawn_check1_server(db: FirestoreDb, authorization_url: String) -> String {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
            .await
            .unwrap();
        let addr = listener.local_addr().unwrap();
    ...
    async fn end_to_end_authorized_check() {
        ...
        let check1_url = spawn_check1_server(db.clone(), authorization_url).await;
        ...
        seed_function_group_attributes(&db, &group_id, &{
            ...
            m.insert("department".to_string(), json!("engineering"));
            m
        })
        .await;
